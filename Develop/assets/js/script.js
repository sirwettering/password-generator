...
var generatePassword = function() {
  ...
  while(goodInput) {
    userInput = window.prompt("Please select criteria for password (you can select more than one) (use space): 1=lowercase 2=uppercase 3=numeric 4=special characters.\n");
    if (!userInput) {
    ...
    for (var i = 0; i < userInput.length; i++) {
      ...
      switch(inputCriteriaInteger) {
        ...
        default: 
          ...
          // If not !inputCriteriaInteger means that we have non integers here.
          else { 
...
function writePassword() {
  ...
  // Call Function to Validate Desired Length
  var pLength = passLength();
  //The passLength() Function with Succeeded.
 
  //Use displayPassword to Store the Output of finalPassword.
  
  //Pass the Valid userInput and Length selected by the user.
